...
public class EvolutionExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> GenericJDBCException(org.hibernate.exception.GenericJDBCException ex,
		...
		String mensagemUsuario = ExceptionUtils.getRootCauseMessage(ex).replaceAll("SQLException: ", "");
		String mensagemDesenvolvedor = ExceptionUtils.getRootCauseMessage(ex).replaceAll("SQLException: ", "");

		List<Erro> erros = Arrays.asList(new Erro(this.getMensangemOracle(mensagemUsuario), mensagemDesenvolvedor));
		return handleExceptionInternal(ex, erros.get(0), new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
	}

	@ExceptionHandler({ com.evolution.service.seguranca.NegocioExceptionService.class })
	public ResponseEntity<Object> NegocioExceptionService(com.evolution.service.seguranca.NegocioExceptionService ex,
			WebRequest request) {
		String mensagemUsuario = ExceptionUtils.getRootCauseMessage(ex).replaceAll("Erro: ", "");
		String mensagemDesenvolvedor = ExceptionUtils.getRootCauseMessage(ex).replaceAll("Erro: ", "");
		List<Erro> erros = Arrays
				.asList(new Erro(this.getMensangemNegocioExceptionService(mensagemUsuario), mensagemDesenvolvedor));
		return handleExceptionInternal(ex, erros.get(0), new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
	}
